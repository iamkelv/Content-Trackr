...
export default function DevToUpdateForm(props) {
  const {
    id: idProp,
    devTo: devToModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = devToRecord
      ? { ...initialValues, ...devToRecord }
      : initialValues;
  ...
  const [devToRecord, setDevToRecord] = React.useState(devToModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(DevTo, idProp)
        : devToModelProp;
      setDevToRecord(record);
    };
    queryData();
  }, [idProp, devToModelProp]);
  React.useEffect(resetStateValues, [devToRecord]);
  const validations = {
    profile_name: [],
    article_title: [],
    pub_date: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            DevTo.copyOf(devToRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DevToUpdateForm")}
      {...rest}
    >
      <TextField
        label="Profile name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || devToModelProp)}
